...
> **關於基礎模板**
> 本專案以 [UranusLin/springboot-jwt-template](https://github.com/UranusLin/springboot-jwt-template) 作為起始模板，
> 該模板提供 Spring Security + JWT 的基礎認證架構（對應 V1 migration）。
> V2～V7 的資料模型設計、RBAC 與細粒度授權機制、Redis 快取層，
> 以及所有業務邏輯與 API 端點，皆為本人設計與實作。

> **版本說明**
> 初版（必修階段）聚焦於跨院、跨科的病歷資訊不同步問題。
> 本版（進階階段）並非在初版上修改，而是重新設計——
> 擴充了病人自主授權模型，並延伸至跨國就醫的使用情境。
